...
compare_check.set<-function (RM_set_1, RM_set_2, DM_set_1, DM_set_2, DM_mesh_1, 
          DM_mesh_2) 
{
  dim1 <- dim(RM_set_1)[1]
  dim2 <- dim(RM_set_2)[1]
  ...
  AM_mesh_1 <- rotmesh.onto(DM_mesh_1, DM_set_1, RM_set_1_sc, 
                            adnormals = FALSE, scale = FALSE, reflection = FALSE)
  AM_mesh_2 <- rotmesh.onto(DM_mesh_2, DM_set_2, RM_set_2_sc, 
                            adnormals = FALSE, scale = FALSE, reflection = FALSE)
  ...
  AM_rot <- rotmesh.onto(AM_mesh, rbind(AM_mesh_1$yrot, AM_mesh_2$yrot), 
                         ...
                         reflection = FALSE)
  check_dist <- abs(sum(projRead(rbind(RM_set_1_sc, RM_set_2_sc), 
                                 AM_rot$mesh)$quality))
  procr_dist <- sin(angle.calc(AM_rot$yrot, rbind(RM_set_1_sc, 
                                                     RM_set_2_sc)))
  eucl_dist <- sum(sqrt(rowSums((AM_rot$yrot - rbind(RM_set_1_sc, 
                                                     RM_set_2_sc))^2)))
  check_1 <- sum(sqrt(rowSums((AM_rot$yrot[1:dim1, ] - RM_set_1_sc)^2)))
  check_2 <- sum(sqrt(rowSums((AM_rot$yrot[(dim1 + 1):(dim1 + 
                                                         dim2), ] - RM_set_2_sc)^2)))
  procr_dist_1 <- sin(angle.calc(AM_rot$yrot[1:dim1, ],RM_set_1_sc))
  procr_dist_2 <- sin(angle.calc(AM_rot$yrot[(dim1 + 1):(dim1 + dim2), ], RM_set_2_sc))
  single_l_1 <- sqrt(rowSums((AM_rot$yrot[1:dim1, ] - RM_set_1_sc)^2))
  single_l_2 <- sqrt(rowSums((AM_rot$yrot[(dim1 + 1):(dim1 + 
                                                        dim2), ] - RM_set_2_sc)^2))
  out <- list(SF1 = SF1, SF2 = SF2, RM_set_1_sc = RM_set_1_sc, 
              RM_set_2_sc = RM_set_2_sc, AM_model = AM_rot, dist_from_mesh = check_dist, 
              eucl_dist_1 = check_1, eucl_dist_2 = check_2, procr_dist = procr_dist, 
              procr_dist_1 = procr_dist_1, procr_dist_2 = procr_dist_2, 
              eucl_dist = eucl_dist, single_l_1 = single_l_1, single_l_2 = single_l_2)
  return(out)
}